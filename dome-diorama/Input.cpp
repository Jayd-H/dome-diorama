#include "Input.h"

#include <cstring>

#include "Debug.h"

Input::Input()
    : mouseX(0.0),
      mouseY(0.0),
      lastMouseX(0.0),
      lastMouseY(0.0),
      mouseDeltaX(0.0),
      mouseDeltaY(0.0),
      scrollDelta(0.0),
      firstMouse(true) {
  (void)std::memset(currentKeys, 0, sizeof(currentKeys));
  (void)std::memset(previousKeys, 0, sizeof(previousKeys));
  (void)std::memset(currentMouseButtons, 0, sizeof(currentMouseButtons));
  (void)std::memset(previousMouseButtons, 0, sizeof(previousMouseButtons));
}

void Input::update() {
  mouseDeltaX = mouseX - lastMouseX;
  mouseDeltaY = mouseY - lastMouseY;
  lastMouseX = mouseX;
  lastMouseY = mouseY;

  if (firstMouse) {
    mouseDeltaX = 0.0;
    mouseDeltaY = 0.0;
    firstMouse = false;
  }
}

void Input::endFrame() {
  (void)std::memcpy(previousKeys, currentKeys, sizeof(currentKeys));
  (void)std::memcpy(previousMouseButtons, currentMouseButtons,
                    sizeof(currentMouseButtons));
  scrollDelta = 0.0;
}

void Input::onKey(int key, int scancode, int action, int mods) {
  (void)scancode;
  (void)mods;

  if ((key < 0) || (key > GLFW_KEY_LAST)) {
    return;
  }

  if (key == GLFW_KEY_ENTER) {
    Debug::log(Debug::Category::INPUT, "Enter key, action: ", action);
  }

  if (action == GLFW_PRESS) {
    currentKeys[key] = true;
  } else if (action == GLFW_RELEASE) {
    currentKeys[key] = false;
  } else {
    // GLFW_REPEAT - do nothing
  }
}

void Input::onMouseMove(double xpos, double ypos) {
  mouseX = xpos;
  mouseY = ypos;
}

void Input::onMouseButton(int button, int action, int mods) {
  (void)mods;

  if ((button < 0) || (button > GLFW_MOUSE_BUTTON_LAST)) {
    return;
  }

  if (action == GLFW_PRESS) {
    currentMouseButtons[button] = true;
  } else if (action == GLFW_RELEASE) {
    currentMouseButtons[button] = false;
  } else {
    // Unknown action - do nothing
  }
}

void Input::onScroll(double xoffset, double yoffset) {
  (void)xoffset;

  Debug::log(Debug::Category::INPUT, "Scroll received: ", yoffset);
  scrollDelta = yoffset;
}